        ...
        with open(outfile, "w") as output:
            for seq_record in SeqIO.parse(input_handle, "genbank"):
                ...
                for seq_feature in seq_record.features:
                    if hasGeneIdentifier(seq_feature):
                        locus_tag = getGeneIdentifier(seq_feature)
...
def run_blastAll(faaFiles, workingDir, title="transclust", dbName="db", evalue="0.01", num_threads="4"):
    ...
    print "Running blast"
    cmd = ["blastp", "-db", db, "-query", merged_faa, "-evalue",  evalue, "-outfmt", "6", "-num_threads", num_threads, "-out", os.path.join(workingDir,"merged.blast2p")]
    print " ".join(cmd)
    subprocess.call(cmd)
...
def getAllFiles(workingDir, ending, exclude=None):
    ...
    for f in os.listdir(workingDir):
        if os.path.isfile(os.path.join(workingDir, f)) and os.path.splitext(f)[1] == ending:
            if not exclude or not os.path.abspath(os.path.join(workingDir,f)) == os.path.abspath(exclude):
	      if exclude:
                print "f:", os.path.abspath(f), os.path.abspath(exclude) 
...
def hasGeneIdentifier(seq_feature):
    return 'locus_tag' in seq_feature.qualifiers and len(seq_feature.qualifiers['locus_tag']) == 1
        ...
        for seq_record in SeqIO.parse(input_handle, "genbank"):
            if not seq_record.id in geneFamilies:
                ...
                for seq_feature in seq_record.features:
                    ...
                    if not validFeature(seq_feature):
                        continue
                    if not locus_tag in geneFamilies[seq_record.id]:
                        print "Missing gene %s, assigning 0 homology!" % locus_tag
                        geneID = 0
                    else:
                        geneID = geneFamilies[seq_record.id][locus_tag]
                   ...
                   # locusTag = seq_feature.qualifiers["locus_tag"][0]
                    positionMap[position] = ("%d\t%s\t%s\t%s\t%s\t%s\t%s\n" % (geneID, strand, "?", geneName, product, locus_tag, "unknown"), note)
